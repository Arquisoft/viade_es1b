...
* The user who wants to add a friend in a group has to go to the Friends tab
* In order to add a friend to a group you have to have friends. When you have selected a friend and a group also selected, click the add to group button
* The service will write in the pod a JSON with all the data of the group
** If it works, the graphical interface is updated
** If it doesn't work, a message is sent with an error

[plantuml, "Add a friend to a group",png]

----
title Add a friend in a group

actor USER #red
participant UI #yellow
control GoupService #green
database POD #pink


USER->UI:Click Friends
UI->GoupService:Clik in "Add group" addGroup (group,friend)
GoupService->POD: Write JSONGroup in POD

alt successful
POD->GoupService: Friend added a group
POD-->UI: Update UI
else failure
POD->GoupService: Friend not added a group
POD-->UI: Failure notification
end
...
|HIKER | Is the users who will use the application.\nThe color that identifies the hikers is red |<back:#FF0000>    </back> |
|UIAplication | They are the GUIs\n with which hikers can interact. All GUIs are yellow |<back:yellow>     </back> |
|Data service | It's a service that allows you to add friend a group.The color that identifies the services is green. |<back:green>    </back> |
|SOLID|Pink color identify your personal\n server (pink)|<back:pink>    </back> |
endlegend
----
